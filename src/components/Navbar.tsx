...
const Navbar = () => {
  ...
  return <nav className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700/50 shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 px-[3px] mx-0 my-0 py-[34px]">
          ...
          <div className="flex items-center space-x-3">
                <div className="flex flex-col items-center justify-center bg-gradient-to-br from-blue-600 to-blue-800 dark:from-blue-500 dark:to-blue-700 rounded-xl px-3 py-2 shadow-lg group-hover:scale-105 transition-transform duration-300 bg-slate-50">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map(item => <Button key={item.path} variant={location.pathname === item.path ? "default" : "ghost"} size="sm" onClick={() => navigate(item.path)} className={`text-sm transition-all duration-200 ${location.pathname === item.path ? 'medical-gradient text-white shadow-lg' : 'hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-700 dark:hover:text-blue-300'}`}>
                {item.label}
              </Button>)}
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            {!isMobile && <div className="flex items-center space-x-3 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 px-4 border border-blue-200 dark:border-blue-700 rounded-3xl py-px">
            ...
            {isMobile && <Button variant="ghost" size="sm" onClick={toggleTheme} className="p-2">
                {isDark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
              </Button>}

            {/* Settings - Desktop Only */}
            {!isMobile && <Button variant="ghost" size="sm">
        ...
        {mobileMenuOpen && <div className="md:hidden py-4 border-t border-gray-200 dark:border-gray-700">
            <div className="space-y-2">
              {navItems.map(item => <Button key={item.path} variant={location.pathname === item.path ? "default" : "ghost"} className="w-full justify-start" onClick={() => {
            navigate(item.path);
